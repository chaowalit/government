...
class CalculateMiddlePriceController extends AdminMsgController{
	...
	public function index(){
		...
		$CalculateMiddlePrice = $CalculateMiddlePrice->getCalculateMiddlePriceAll();
		$data = array(
			'menu_name' => $this->menu_name,
			'calculate_middle_price' => $CalculateMiddlePrice,
		);

		$this->render_view('admin/calculate_middle_price/main', $data, $this->menu_nav, 1);
	}
	...
	public function form(){
		...
		$this->render_view('admin/calculate_middle_price/form', $data, $this->menu_nav, 1);
	}

	public function edit($id = ''){
		$CalculateMiddlePrice = new CalculateMiddlePrice;
		$result = $CalculateMiddlePrice->getDataById($id);
	...
	public function save(Request $Requests){
		...
		if($Requests->get('edit_id', '') == ""){
			...
			$CalculateMiddlePrice = new CalculateMiddlePrice;
			$CalculateMiddlePrice->save_data($data, $Requests->get('edit_id', ''));

			$Requests->session()->flash('bg_color', 'success');
			$Requests->session()->flash('msg', "ทำรายการบันทึกข้อมูล สำเร็จแล้ว");
			return redirect('admin/cal_mid_price/edit/'.$Requests->get('edit_id', ''));
		}
	...
	public function delete($id = ''){
		...
		$CalculateMiddlePrice->delete_row($id);

		return redirect('admin/cal_mid_price');
	}
}
?>
